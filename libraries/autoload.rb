...
if defined?(ChefCompat)
  ...
  fail 'Version file not in correct format' unless version_rb =~ /VERSION\s*=\s*'([^']+)'/
  cookbook_version = Regexp.last_match(1)

  if CompatResource::VERSION != cookbook_version
    fail "compat_resource gem version #{CompatResource::VERSION} was loaded as a gem before compat_resource cookbook version #{cookbook_version} was loaded. To remedy this, either update the cookbook to the gem version, update the gem to the cookbook version, or uninstall / stop loading the gem so early."
  end
else
