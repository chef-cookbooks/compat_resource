        ...
        module Mixin
          ...
          module ParamsValidate
            ...
            def explicitly_allows_nil?(key, validation)
              validation.key?(:is) && _pv_is({ key => nil }, key, validation[:is], raise_error: false)
            end
            ...
            def _pv_required(opts, key, is_required = true, explicitly_allows_nil = false)
              if is_required
                ...
                fail Exceptions::ValidationFailed, "Required argument #{key.inspect} is missing!"
              end
              true
            end
            ...
            def _pv_cannot_be(opts, key, predicate_method_base_name)
              ...
              unless value.nil?
                Array(predicate_method_base_name).each do |method_name|
                  ...
                  next unless value.respond_to?(predicate_method)
            ...
            def _pv_name_property(opts, key, is_name_property = true)
              if is_name_property
                if opts[key].nil?
                  fail CannotValidateStaticallyError, 'name_property cannot be evaluated without a resource.' if self == Chef::Mixin::ParamsValidate
            ...
            def _pv_is(opts, key, to_be, raise_error: true)
              ...
              passed = to_be.any? do |tb|
                case tb
                when Proc
                  fail CannotValidateStaticallyError, 'is: proc { } must be evaluated once for each resource' if self == Chef::Mixin::ParamsValidate
            ...
            def _pv_coerce(opts, key, coercer)
              if opts.key?(key.to_s)
                fail CannotValidateStaticallyError, 'coerce must be evaluated for each resource.' if self == Chef::Mixin::ParamsValidate
            ...
            class SetOrReturnProperty < (defined?(::Chef::Mixin::ParamsValidate::SetOrReturnProperty) ? ::Chef::Mixin::ParamsValidate::SetOrReturnProperty : Chef::Property)
              def get(resource)
                ...
                set_value(resource, value) unless is_set?(resource)
                value
              end
